...
class ActiveReceipts(unittest.TestCase):
    ...
    def test_live_receipts_require_both_consumers_and_fence_identity(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            early=dict(first=5000,last=5000)
            write('exotica-active-scenes.csv',dict(owned,model_checks='2',model_bytes='16'))
            with self.assertRaisesRegex(ValueError,'lease counts'):verify(temp,[source],current,2,[])
            with self.assertRaisesRegex(ValueError,'explicit early'):verify(temp,[source],current,2,[],early=early)
            self.assertTrue(verify(temp,[source],current+'MIDZ_ENDPOINT_EARLY=1\n',2,[],early=early)['passed'])
            write('exotica-active-scenes.csv',dict(owned,model_checks='1',model_bytes='8'))
            with self.assertRaisesRegex(ValueError,'lease counts'):verify(temp,[source],current+'MIDZ_ENDPOINT_EARLY=1\n',2,[],early=early)
            write('exotica-active-scenes.csv',owned)
