...
def combine_patches(paths, destination):
    combined = {}
    for path in paths:
        for addr, values in read_patch(path).items():
            if addr in combined and combined[addr] != values:
                raise ValueError(f"conflicting game patches at word {addr:05X}: {path}")
            combined[addr] = values
    text = "# Combined game patches; generated by the launcher.\n"
    for path in paths:
        text += f"# Source: {Path(path).resolve()}\n"
    for addr, (old, new) in combined.items():
        old_text = "*" if old is None else f"{old:08X}"
        text += f"{addr:05X} {old_text} {new:08X}\n"
