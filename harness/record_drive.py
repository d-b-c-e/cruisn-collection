...
def main(argv=None):
    ...
    try:
        ...
        args.offroad_trial = offroad_distance.configure(args, rom, {})
    ...
    if args.offroad_trial:
        print(f"EXPERIMENTAL Off Road global distance: {args.offroad_trial}.",flush=True)
...
def record(args, state, card, rom, output):
    proc, hwnd = run_rig.launch_game_async(
        ...
        record_exotica_trial=args.exotica_trial,record_offroad_trial=args.offroad_trial)
    if args.title:
