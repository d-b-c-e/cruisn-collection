...
def configure(args,rom,settings):
    ...
    if mode is None:
        if first is not None or last is not None:raise ValueError('host frame bounds require an explicit mode')
    ...
    for key,value in [('MIDV_WORLD_HOST_FIRST',first),('MIDV_WORLD_HOST_LAST',last)]:
        if value is None:settings.pop(key,None)
        else:settings[key]=str(value)
    return dict(mode=mode,first=first,last=last)
