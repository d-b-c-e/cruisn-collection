...
def snapshot(directory, frame, kind='host'):
    ...
    return dict(frame=frame, phase=kind, scene=p['scene'], generation=p['generation'], pages=len(p['pages']),
...
def verify_live(directory, scenes, captures, text, active=False):
    ...
    previous = 0
    for i, (sent, received) in enumerate(zip(producer, gpu)):
        source = scenes[i//phases]
        late = i % phases != 0
    ...
    return dict(passed=True, scenes=len(scenes), snapshots=sampled,
                scope='Private material upload/ownership; general lifetime/handover acceptance remains separate.')
