...
def add_arguments(parser):
    parser.add_argument('--zeus-depth-mirror',choices=('off','observe'),help='diagnostic original-only private D32F mirror; displayed target stays original')
...
def configure(args,rom,settings,frames):
    ...
    if mode is not None:
        if mode not in ('off','observe') or not getattr(args,'candidate',None):raise ValueError('depth mirror requires an explicit candidate')
        ...
        settings.update(zip(KEYS,('1',str(values[0]),str(values[1]),values[2] or '')))
    if 'MIDZ_DEPTH_MIRROR' not in settings:
        if any(k in settings for k in KEYS[1:]):raise ValueError('orphan depth mirror bounds')
        return None
    if (rom!='crusnexo' or settings['MIDZ_DEPTH_MIRROR']!='1' or settings.get('MIDZ_GL')!='1' or
    ...
    return dict(enabled=True,explicit=mode is not None,first=first,last=last,snapshots=sorted(snapshots))
...
def verify_receipt(trial,text,directory):
    initial=re.findall(r'^MIDZ_DEPTH_MIRROR=1 first=(\d+) last=(\d+) snapshots=(\d+)$',text,re.M)
    ...
    if initial!=[(str(first),str(last),str(len(captures)))] or len(final)!=1 or len(writer)!=1:raise ValueError('missing depth mirror acknowledgments')
    ...
    for row in rows:
        ...
        previous=current
        if (int(row['snapshot'])!=int(frame in captures) or int(row['color_differences']) or int(row['depth_differences']) or
        ...
        if frame in captures:
            ...
            result=compare_buffers(*buffers)
    ...
    return dict(passed=True,frames=count,batches=batches,vertices=vertices,clears=clears,snapshots=results,
        scope='Original-only private color/D32F comparison at requested frames. No farther scenery, general handover or full visual acceptance.')
