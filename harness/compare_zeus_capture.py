...
def compare(reference,candidate,frame):
    ...
    original,changed=signatures(reference),signatures(candidate)
    ...
    return dict(verdict='PASS' if only_additions and resources_equal else 'FAIL',
...
def main():
    ...
    args=ap.parse_args();report=compare(args.reference,args.candidate,args.frame)
