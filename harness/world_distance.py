"""Explicit World 2.4/2.5 global-distance experiments; no launcher defaults.
...
SUPPORTED_ROMS = ('crusnwld24', 'crusnwld')
MAXIMUM_LEAD = 12
...
def add_arguments(parser):
    parser.add_argument('--world-far', type=int, choices=FAR_VALUES,
                        help='explicit World 2.4/2.5 global distance trial; 80000 is the original limit')
...
def configure(args, rom, settings):
    if args.world_far is None:
        ...
        return None
    if rom not in SUPPORTED_ROMS:
        raise ValueError('global distance experiment requires World 2.4/2.5')
...
def compose(base, destination, far):
    ...
    delta.write_text('# World 2.4/2.5 global distance trial; revision-guarded native virtual reciprocals required.\n' +
