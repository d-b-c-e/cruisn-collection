...
| **G5** | Settings UX: per-game labels unclear + screen cluttered | ☐ | M | 🙋 | **USER DESIGN LANDED 2026-08-29** (docs/settings-redesign.md): global-only settings stay on the main menu bottom row; selecting a game opens a per-game submenu (PLAY + that game's settings - sensitivity, curve, shifter, volume, free play). Ready to implement once the perf regression (G10) is closed and volume/free-play bytes are fully calibrated. |
