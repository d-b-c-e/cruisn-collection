...
| ID | Item | Status | Effort | Auto | Notes |
|----|------|--------|--------|------|-------|
| **G1** | Launch "ding" — STILL present, all four games | ☐ | M | ⚠️ | **CONVICTED 2026-08-25: the FFB plugin.** User A/B (rig/ding-ab.bat): normal launch dings, launch with dinput8.dll renamed away does NOT — all four games. Next: read FFBArcadePlugin source (GPL) for the beep/MessageBox in its init path; fix via config, a patched plugin build, or interception. Pairs with G8 (same plugin's teardown). |
...
| **G3** | Exotica car-select stats text illegible | ☐ | M | ⚠️ | RECLASSIFIED 2026-08-25: user captured car-select stats via BOTH renderers (GL overlay and MIDZ_GL=0 d3d) — text equally illegible in MAME's own output → UPSTREAM zeus2 emulation bug, not our overlay. Candidate for an upstream-grade zeus2 fix like the road/translucency backports. |
...
| **G5** | Settings UX: per-game labels unclear + screen cluttered | ☐ | M | 🙋 | CONFIRMED with user 2026-08-25: the model is "highlight a game card, then SETTINGS tunes that game" — works, "though not intuitive at all". Rework: split into submenus (DISPLAY / WHEEL / PER-GAME / TELEMETRY) with an explicit "TUNING: <game>" header on the per-game page, or move per-game tuning behind each game card. Design with the user. |
...
| **G7** | Manual mode: H-pattern gear engagement mis-detected | ☐ | M | ⚠️ | **ROOT CAUSE FOUND 2026-08-25**: MAME CONF port "Shifter Type" defaults to "Buttons (sticky)"; the rig has a REAL H-pattern shifter and MAME offers "H-Pattern" (0x00) we never set — user confirmed sticky semantics exactly (2nd registers on moving AWAY to neutral). Fix: write CONF=H-Pattern into per-game cfg when the wizard has gears bound; "Sequential" (0x05) = B7 for paddles, zero custom code. World-hides-manual / Exotica-auto-select: cabinet DIPs (crusnwld "Cabinet: Upright/Sitdown", crusnexo "Cabinet: Stand Up/Sit Down", both default shifterless) — set Sitdown via cfg and retest. |

| **G8** | FFB forces not released when a game closes | ☐ | S | ⚠️ | NEW 2026-08-25, almost certainly OUR fast-exit regression: TerminateProcess skips the FFB plugin's DLL_PROCESS_DETACH cleanup — the same teardown that used to AV was also what disarmed the wheel. Interim: run_rig fires a Stop-FFB action automatically after process exit. Real fix: patched plugin build (pairs with G1). |
| **G9** | B3 volume bytes: first data points | ◐ | M | ⚠️ | 2026-08-25 NVRAM diffs vs pre-snapshots: crusnwld byte 0x93C = master volume (user's 11 = 0x0B, byte-exact). offroadc/crusnusa/crusnexo diffs captured (mirror + checksum structure to decode; games have separate MINIMUM and master volume fields — user's earlier trials ambiguous, tonight's known). Free-play flags in the same diffs (World/Off Road/Exotica set at the wheel — C1 effectively done). |
