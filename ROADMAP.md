...
| **B2** | Telemetry Phase B — RPM | ◐ | M | ⚠️ | **Leading candidate 0x0F0C0 (mirror 0x0DB67)** found 2026-08-23 via a FINE-cadence recapture (every 3 frames): bounded 1.1–1.54, step-drops at shift points (tach-like). NOT wired — it's flat while speed climbs in 1st gear (not cleanly proportional) and has no odometer-style cross-check. Needs a one-glance on-screen TACHOMETER correlation at the wheel to confirm + scale. Detector is turnkey. |
...
| **B5** | Sky-streak polish — vertical-gradient sky fill in margins | ☑ | — | — | **DONE 2026-08-23.** Present-pass fetch_smooth vertically gaussian-blurs unwritten 16:9 margin pixels → clean sky gradient, no horizontal streaks; horizon reads as soft haze. Gated uMargin>0 (inert in exact/4:3), exact still 100.0000%. Proof: results/proof/vunit-sky-margin-smoothing.png. |
