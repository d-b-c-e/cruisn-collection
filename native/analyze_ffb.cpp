...
int main(int argc, char **argv) {
    if (argc != 6) { std::fprintf(stderr, "trace profiles_dir profile_id strength output_csv\n"); return 2; }
    ...
    while (std::getline(input, line)) {
        ...
        try {
            long long ms = std::stoll(t); end_ms = std::max(end_ms, ms);
            if (!motor_columns && name != "wheel") continue;
            int raw = std::stoi(value);
            if (!motor_columns) { raw &= 255; if (raw >= 128) raw -= 256; }
    ...
    profile.shaper.strength = strength / 2; profile.shaper.invert = false;
    dbce::force::Shaper shaper(profile.shaper);
    dbce::force::RiseDetector detector;
    std::ofstream csv(argv[5]);
    if (!csv) return 1;
    csv << "ms,motor_byte,normalised,shaped,impact_candidate,arrival,rise,rumble_request\n";
    ...
    for (long long ms = 0; ms <= end_ms; ms += 4) {
        ...
        if (event) events.push_back(ms);
        csv << ms << ',' << current << ',' << normal << ',' << shaped << ',' << int(event)
            << ',' << detector.last_arrival << ',' << detector.last_rise << ',' << rumble << '\n';
        peak = std::max(peak, std::abs(double(shaped))); sum_squares += shaped * shaped; ++ticks;
        if (strength >= 2 && std::abs(shaped) >= (strength / 2) / 50.f * .9999f) ++full_ticks;
    }
    csv.close();
    if (!csv) return 1;
